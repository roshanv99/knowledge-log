---
name: manim-scene
description: Writes Manim Community v0.21 code for a vertical 2b explainer reel from an approved beat script with measured narration durations, and fixes it from render errors, layout-audit failures or visual-review feedback. Used by the knowledge-log manim spike.
---
...
Before your first scene, you may read `spike/reel_scene.py` to see exactly what the base class does.
```python
from manim import *
from spike.reel_scene import ReelScene, labeled_box, SAFE_LEFT, SAFE_RIGHT, SAFE_TOP, SAFE_BOTTOM, SAFE_WIDTH, SAFE_CENTER_X, TITLE_Y, CAPTION_Y
...
## Fixing
Write the module to `<work>/scene.py`. When a render, the lint, the layout audit or the visual check fails, edit `scene.py` in place.
