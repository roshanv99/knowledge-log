...
## Steps
1. `uv run --project pipeline --extra reels kl reel start` (add `--document "<name>"` if
...
3. When the run is over, tell the user the stop reason, how many reels were made, and the review
   path it printed.
...
- Use only `kl reel`, Read, Write/Edit under `.kl/`, and subagents. A scene only draws: no files,
