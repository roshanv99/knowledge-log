---
name: reel-script
description: Writes the narration and per-beat visual plan for a 2b manim explainer reel from one chunk of the user's notes. Used by the knowledge-log reel pipeline (kl reel) before any scene code is written.
---
...
## What you produce
Write it to the path your brief gives, in this shape:
```json
...
- The narration is read by a TTS voice (Kokoro). Write for the ear:
...
- The pages often cover several ideas. Pick the single most valuable one to teach, and leave the rest out.
...
## Revisions
When you get critic issues, edit the script file to fix every one of them. Keep everything else that worked, unless a fix requires changing it.
